...
for subnet_data in input_data['networks']:
    subnet[subnet_data['name']+"_subnet"] = {'type': 'OS::Neutron::Net','depends_on':'','properties':{'network_id':'','cidr':'','gateway_ip':'','dns_nameservers':[],'allocation_pools':[{'start':'','end':''}]}}
    subnet[subnet_data['name']+"_subnet"]['depends_on'] = subnet_data['name']
    subnet[subnet_data['name']+"_subnet"]['properties']['network_id'] = "{ get_resource: %s }" % subnet_data['name']
